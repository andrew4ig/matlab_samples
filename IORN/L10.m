...
for Time=1:m(ms)
    ...
    Ptr=sum(G2(1:Np/2))/sum(G2);
        
    Pre=sum(G2(Np/2+1:Np))/sum(G2);
    ...
    if m(k)== Time
        ...
        text(3.5e-8,-1e4, sprintf('Ptr=%3.2g',    round(Ptr*100)/100),          'Interpreter','latex')
        text(3e-8,-1e4,   sprintf('Pre=%3.2g',    round(Pre*100)/100),          'Interpreter','latex')
